...
test('applies the composed matrix to the real site', async ({ context, extPage }) => {
  ...
  try { await page.goto('https://teams.scrumlaunch.com/time-tracker', { waitUntil: 'load', timeout: 30_000 }); }
  catch (e) { test.skip(true, `site unreachable: ${e.message}`); }
  ...
  const expected = [0, 1, 2].map(i => Math.round(255 * Math.min(1, Math.max(0, matrix[i*5]*p0[0]/255 + matrix[i*5+1]*p0[1]/255 + matrix[i*5+2]*p0[2]/255 + matrix[i*5+4]))));
